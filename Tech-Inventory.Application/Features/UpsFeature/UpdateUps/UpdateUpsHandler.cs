...
public class UpdateUpsHandler : IRequestHandler<UpdateUpsRequest, ApiResponse>
{
    ...
    public UpdateUpsHandler(ITechInventoryDB context, IMapper mapper, IUnitOfWork unitOfWork)
    {
        _context = context;
        _mapper = mapper;
    ...
    public async Task<ApiResponse> Handle(UpdateUpsRequest request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            return ResponseHandler.GetAppResponse(type, new UpdateUpsResponse { Id = ups.Id, Message = "Ups has updated" });
        }
        catch (Exception ex)
        {
